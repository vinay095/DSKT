...
const ELEMENT_TYPE_STYLES: Record<string, CategoryVisualStyle> = {
  ...
  // Meeting
  table: CATEGORY_STYLES.meeting_room,
  table2: {
    fill: '#57534E',
    stroke: '#292524',
    fillOpacity: 0.45,
  ...
  // Cafeteria
  'dining-table-6': CATEGORY_STYLES.cafeteria,
  'bar-counter': {
    fill: '#A8A29E',
    stroke: '#57534E',
    fillOpacity: 0.55,
  ...
  kitchenFridge: CATEGORY_STYLES.cafeteria,
  kitchenSink: CATEGORY_STYLES.cafeteria,
  'kitchen-sink-corner': CATEGORY_STYLES.cafeteria,
  'countertop-corner': CATEGORY_STYLES.cafeteria,
  'fume-hood-md': CATEGORY_STYLES.cafeteria,
  // Restroom
...
export function stylizeCatalogSvgMarkup(
  ...
  out = out.replace(
    /fill\s*=\s*["']#(?:fff|ffffff|FFFFFF|000|000000)["']/gi,
    `fill="${style.fill}"`,
  );
  out = out.replace(
    /stroke\s*=\s*["']#(?:000|000000|494949)["']/gi,
    `stroke="${style.stroke}"`,
  );
